//================================================================
// Port SDL1 and RP2040 by ackerman
//
...
#include "gbConfig.h"
#ifdef use_lib_hdmi
 #include <PicoDVI.h>
 static const struct dvi_serialiser_cfg wareshare_pizero_cfg = {
  //Wareshare pizero
 ...
 DVIGFX8 gb_display_hdmi(DVI_RES_320x240p60, false, wareshare_pizero_cfg);  //simple buffer
 //DVIGFX8 gb_display_hdmi(DVI_RES_320x240p60, true, wareshare_pizero_cfg);  //double buffer requiere gb_display_hdmi.swap
 uint8_t *gb_buffer_hdmi; //pointer HDMI 320x240
#else
 #ifdef use_lib_vga
...
//bitluni #ifdef use_lib_vga360x200x70hz_bitluni               
//bitluni unsigned char gb_vga_videomode_cur= video_mode_360x200x70hz_bitluni; //0
//bitluni#else
//bitluni #ifdef use_lib_vga360x200x70hz_bitluni_apll_fix
//bitluni  unsigned char gb_vga_videomode_cur= video_mode_360x200x70hz_bitluni_apll_fix; //1
//bitluni #else
//bitluni  #ifdef use_lib_vga320x200x70hz_bitluni
//bitluni   unsigned char gb_vga_videomode_cur= video_mode_vga320x200x70hz_bitluni; //2
//bitluni  #else
//bitluni   #ifdef use_lib_vga320x200x70hz_fabgl
//bitluni    unsigned char gb_vga_videomode_cur= video_mode_vga320x200x70hz_fabgl; //3
//bitluni   #else
//bitluni    #ifdef use_lib_vga320x200x70hz_bitluni_apll_fix
//bitluni     unsigned char gb_vga_videomode_cur= video_mode_vga320x200x70hz_bitluni_apll_fix; //4
//bitluni    #else 
//bitluni     #ifdef use_lib_vga320x240x60hz_bitluni
//bitluni      unsigned char gb_vga_videomode_cur= video_mode_vga320x240x60hz_bitluni; //5
//bitluni     #else
//bitluni      #ifdef use_lib_vga320x240x60hz_fabgl
//bitluni       unsigned char gb_vga_videomode_cur= video_mode_vga320x240x60hz_fabgl; //6
//bitluni      #else 
//bitluni       #ifdef use_lib_vga320x240x60hz_bitluni_apll_fix
//bitluni        unsigned char gb_vga_videomode_cur= video_mode_vga320x240x60hz_bitluni_apll_fix; //7
//bitluni       #endif
//bitluni      #endif
//bitluni     #endif
//bitluni    #endif
...
//bitluni #endif
//bitluni#endif 
unsigned char gb_vga_videomode_cur=5; //solucion temporal bitluni 320x240

unsigned char gb_vga_videomode_cur_menu=5; //El que seleccionas en menu OSD
...
#endif
*/
const unsigned int *gb_ptrVideo_cur; //bitluni pendiente revisar
 ...
 */
  ...
  // {
  //  gb_display_hdmi.drawPixel(x,y,auxC);
  // }
  //}
  //memset(gb_buffer_hdmi,auxC,76800); //320x240
  unsigned int aux= auxC;
  unsigned int a32= (aux<<24)|(aux<<16)|(aux<<8)|aux;
 ...
 #else
  #ifdef use_lib_vga
   //memset(vga_data_array,auxC,sizeof(vga_data_array));  
   unsigned int aux3= (auxC & 0x07);
   unsigned int aux= ((aux3<<3)|aux3);
   unsigned int a32= (aux<<24)|(aux<<16)|(aux<<8)|aux;
...
void PrepareBWfilterVGA()
{
  ...
  if (gb_vga_8colors == 1)
  {
   ...
   {
    case 0: gb_color_bw[1]= 7; break; //Blanco y Negro
    case 1: gb_color_bw[1]= 2; break; //Verde
    case 2: gb_color_bw[1]= 3; break; //Amarillo naranja
   }
  }
  else
  {
   switch (gb_use_video_filter)
   {
    case 0: gb_color_bw[1]= 63; break; //Blanco y Negro
    case 1: gb_color_bw[1]= 12; break; //Verde
    case 2: gb_color_bw[1]= 11; break; //Amarillo naranja
   }
  } 
 #else
  #ifdef use_lib_vga
...
{  
 ...
 {//8 colores
  ...
  {
   //bitluni gb_const_colorNormal[i]= (i & 0x07) | gb_sync_bits;    
   #ifdef use_lib_hdmi    
    gb_const_colorNormal[i]= (i & 0x07); //No lo necesitamos
   #else
    #ifdef use_lib_vga
     gb_const_colorNormal[i]= (i & 0x07);
    #endif   
   #endif 
  }
 ...
 {//64 colores
  ...
  {
   //bitluni gb_const_colorNormal[i]= (i & 0x3F) | gb_sync_bits;
   #ifdef use_lib_hdmi
    gb_const_colorNormal[i]= (i & 0x07); //No lo necesitamos
   #else
    #ifdef use_lib_vga
...
{          
 ...
 unsigned short int x, y; //unsigned char x, y; //byte x, y;
...
//        refresh_screen();
        ...
        if (!Fassst)                    
        {
            ...
            )
         {
          ...
          //BorraPantalla();
          refresh_screen();
          //Serial.printf("borra:%d\r\n",gb_vga_poll_time_cur);
          //gb_display_hdmi.swap();
         }
         gb_fps_cur++;
        }
        else
...
void ActivarVideoPorTeclas()
{
 ...
 if (gb_use_debug==1){ Serial.printf("Video mode set %d\r\n",video); }
   ...
   //bitluni PrepareColorsBitluniVGA(); //Llamar despues de tener gb_sync_bits  
}
...
//bitluni   a32= a2 | (a3<<8) | (a0<<16) | (a1<<24); //ESP32   
//bitluni   gb_buffer_vga32[pozY][pozX]= a32;   
   #ifdef use_lib_hdmi
    a0= gb_color_bw[((aux>>7)&0x01)];
    a1= gb_color_bw[((aux>>6)&0x01)];
    a2= gb_color_bw[((aux>>5)&0x01)];
    a3= gb_color_bw[((aux>>4)&0x01)];
//    unsigned int pozY_hdmi= pozY;
//    unsigned int pozX_hdmi= pozX;
//    unsigned int auxOfs= (pozY_hdmi<<8)+(pozY_hdmi<<6)+(pozX_hdmi<<2); //320x240
    //gb_display_hdmi.drawPixel(pozX,pozY,a0);
    //gb_display_hdmi.drawPixel(pozX+1,pozY,a1);
    //gb_display_hdmi.drawPixel(pozX+2,pozY,a2);
    //gb_display_hdmi.drawPixel(pozX+3,pozY,a3);
    a32= (a3<<24) | (a2<<16) | (a1<<8) | a0;
    gb_buffer_vga32[pozY][pozX]= a32;
    //gb_buffer_hdmi[auxOfs++]= a0;
    //gb_buffer_hdmi[auxOfs++]= a1;
    //gb_buffer_hdmi[auxOfs++]= a2;
    //gb_buffer_hdmi[auxOfs++]= a3;
   #else
    #ifdef use_lib_vga
     a0= gb_color_bw[(aux>>7)&0x01];
     a1= gb_color_bw[(aux>>6)&0x01];
     a2= gb_color_bw[(aux>>5)&0x01];
     a3= gb_color_bw[(aux>>4)&0x01];    
     //unsigned int auxOfs= ((unsigned int)pozY<<8)+((unsigned int)pozY<<6)+((unsigned int)pozX<<2); //640x240
     a0=(a0<<3)|a0;
     a1=(a1<<3)|a1;
     a2=(a2<<3)|a2;
     a3=(a3<<3)|a3;
     a32= (a3<<24) | (a2<<16) | (a1<<8) | a0;
     gb_buffer_vga32[pozY][pozX]= a32;
     //vga_data_array[auxOfs++]=a0;
     //vga_data_array[auxOfs++]=a1;
     //vga_data_array[auxOfs++]=a2;
     //vga_data_array[auxOfs++]=a3;
    #endif
   #endif 
   ...
   //a32= a0 | (a1<<8) | (a2<<16) | (a3<<24); //PC X86
//bitluni   a32= a2 | (a3<<8) | (a0<<16) | (a1<<24); //ESP32
//bitluni   gb_buffer_vga32[pozY][pozX+1]= a32;
   #ifdef use_lib_hdmi
    //gb_display_hdmi.drawPixel(pozX+4,pozY,a0);
    //gb_display_hdmi.drawPixel(pozX+5,pozY,a1);
    //gb_display_hdmi.drawPixel(pozX+6,pozY,a2);
    //gb_display_hdmi.drawPixel(pozX+7,pozY,a3);   
    a0= gb_color_bw[((aux>>3)&0x01)];
    a1= gb_color_bw[((aux>>2)&0x01)];
    a2= gb_color_bw[((aux>>1)&0x01)];
    a3= gb_color_bw[((aux>>0)&0x01)];
...
void setup() 
{
  ...
  //memset(hdmi_data_array,0,76800);
   ...
   gb_buffer_vga= (unsigned char **)malloc(sizeof(unsigned char *)*240);
   gb_buffer_vga32= (unsigned int **)gb_buffer_vga;
   for (unsigned int i=0;i<240;i++)
   {
    unsigned int ofs= i*320; 
    gb_buffer_vga[i]= &vga_data_array[ofs];   
   }     
  #endif
 #endif 
 ...
 OSDUpdateAnchoAltoVGA(gb_vga_videomode_cur,gb_vga_8colors);
 ...
 #ifdef use_lib_hdmi  
  gb_display_hdmi.begin();
  delay(500);
  gb_buffer_hdmi= gb_display_hdmi.getBuffer();
  vreg_set_voltage(VREG_VSEL); //undervolt overclock gbConfig.h
  delay(100);
  ...
  {
   unsigned int ofs= i*320; 
   gb_buffer_vga[i]= &gb_buffer_hdmi[ofs];   
  }  

  PrepareColorsBitluniVGA(); //No lo necesito
  //for (unsigned short int i=0; i<256; i++) 

  
  int aux=0;
  /*
  switch (gb_vga_8colors)
  {
   case 0:
    for (unsigned short int i=0; i<8; i++)
    { 
     aux= i*64;
     gb_display_hdmi.setColor(i, aux,aux,aux );
    }
    for (unsigned short int i=0; i<8; i++)
    {
     unsigned char r= (i&0x01)*255;
     unsigned char g= ((i>>1)&0x01)*255;
     unsigned char b= ((i>>2)&0x01)*255;
     gb_display_hdmi.setColor(i, r,g,b);
    }
    for (unsigned short int i=8;i<256;i++)
    {
     gb_display_hdmi.setColor(i, 0xFF,0xFF,0xFF);
    }
    break;
   case 1:   
    for (unsigned short int i=0; i<64; i++)
    { 
     unsigned char r= (i&0x03)*4;
     unsigned char g= ((i>>2)&0x03)*4;
     unsigned char b= ((i>>4)&0x03)*4;      
     gb_display_hdmi.setColor(i, r,g,b);
     //aux= i*64;
     //gb_display_hdmi.setColor(i, aux,aux,aux );
    }          
    for (unsigned short int i=64;i<256;i++)
    {
     gb_display_hdmi.setColor(i, 0xFF,0xFF,0xFF);
    }
    break;
  ...
  */
  ...
  //Serial.printf("gb_vga_8colors:%d\r\n",gb_vga_8colors);
  /*
  gb_display_hdmi.setColor(0, 0,0,0 );
  for (int i=1;i<256;i++)
  {
   gb_display_hdmi.setColor(i, 255,255,255 );
  }
  */
  for (unsigned short int i=0; i<8; i++)
  {
  ...
  if (gb_use_debug==1){ Serial.printf("HDMI init\r\n"); }
 #else
  #ifdef use_lib_vga
   initVGA();
 ...
 #ifdef use_lib_log_serial
  //pendiente if (gb_use_debug==1){ Serial.printf("VGA %d\r\n", ESP.getFreeHeap()); }
 #endif
 ...
 #ifdef use_lib_log_serial  
  //pendiente if (gb_use_debug==1){ Serial.printf("KEYBOARD %d\r\n", ESP.getFreeHeap()); }
 #endif
 ...
 //#ifdef use_lib_hdmi
 // gb_setup_end= 0; //No ejecuto nada
 //#else
  gb_setup_end= 1;
 //#endif 
}
 ...
 while(1)
 {
  for (unsigned int i=0;i<1000000000;i++)
  {
   for (unsigned int j=0;j<1000000000;j++)
   {
    ...
    {
     for (unsigned int l=0;l<1000000000;l++)
     {
      for (unsigned int m=0;m<1000000000;m++)
      {
       a= a+1;
      }
      a=a*2;
